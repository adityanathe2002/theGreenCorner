...
const CartPage = () => {
    ...
    const{cartItems, setCartItems} = useContext(plantContext)
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:4343/cart') // Replace with your API endpoint
            .then(response => {
    ...
    const updateCartOnServer = async (updatedCart) => {
        try {
            ...
            await axios.put('http://localhost:4343/cart', updatedCart);
        } catch (error) {
    ...
    const removeItemFromCart = async () => {
        try {
            await axios.delete(`http://localhost:4343/cart/${itemToRemove}`); // Remove item from server
    ...
    const total = subtotal + shippingCharges; // Final total with shipping charges
            ...
            {uniqueProducts === 0 ? (
                ...
                <div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                                    ...
                                    {cartItems.map(item => (
                                        <tr key={item.id} className='border-t-2 border-[#F8F8F8]'>
                                            <td className="w-28 bg-orange-100 h-32 flex items-center gap-3">
                                                <img src={item.image} alt={item.name} className="w-[100%] h-[100%] object-cover rounded-md" 
                        ...
                        <div className="w-full lg:w-1/3 h-96 bg-[#F8F8F8] border border-gray-200 flex flex-col p-4 gap-3 rounded-lg shadow-md">
                            ...
                            <button className="w-full bg-[#163020] text-white py-2 rounded-lg font-semibold hover:bg-green-700" onClick={() => navigate('/checkout', { state: { subtotal, totalItems, uniqueProducts,shippingCharges, total } })}>
                                Proceed to Checkout
                            </button>
                        </div>
                    </div>
